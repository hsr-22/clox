...
static void concatenate() {
    ...
    char* chars = ALLOCATE(char, length + 1); // +1 for the null terminator
    memcpy(chars, a->chars, a->length);
    memcpy(chars + a->length, b->chars, b->length);
    chars[length] = '\0'; // add the null terminator
...
static InterpretResult run() { 
    ...
    #define READ_SHORT() \
        (vm.ip += 2, (uint16_t)((vm.ip[-2] << 8) | vm.ip[-1])) //  reads a 16-bit value from the bytecode stream and advances the ip by 2 bytes.
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_GET_LOCAL: {
                uint8_t slot = READ_BYTE(); //our bytecode instruction set is stack-based
                push(vm.stack[slot]);
                break;
            }
            case OP_GET_GLOBAL: {
            ...
            case OP_JUMP_IF_FALSE: {
                uint16_t offset = READ_SHORT(); // read the offset from the bytecode
                if (isFalsey(peek(0))) vm.ip += offset; // if the top of the stack is falsey, jump
                break;
            }
            case OP_LOOP: {
                uint16_t offset = READ_SHORT();
                vm.ip -= offset; // subtract the offset from the instruction pointer
                break;
            }
            case OP_RETURN: {
                // Exit interpreter.
